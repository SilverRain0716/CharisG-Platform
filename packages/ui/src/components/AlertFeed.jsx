...
export function AlertFeed({ items = [], emptyText = '알림 없음' }) {
  if (items.length === 0) {
    return <div className="rounded-lg bg-surface p-6 text-center text-sm text-ink-400 ring-1 ring-ink-100">{emptyText}</div>;
  }
  return (
    <ul className="divide-y divide-ink-100 rounded-lg bg-surface ring-1 ring-ink-100">
      {items.map((it) => (
