...
def current_user(request: Request, charisg_session: Optional[str] = Cookie(default=None)) -> dict:
    ...
    JWT 자체 유효성(서명/만료) 외에 sessions 테이블의 revoked 도 검사 — 그래야
    로그아웃이 즉시 효과를 가진다. 안 그러면 cookie 가 클라(특히 모바일 Safari)에
    남아있는 동안 7일간 인증 통과되어 로그아웃이 무의미.
    """
    if AUTH_BYPASS:
        return _BYPASS_USER
    ...
    # session revoke 체크 — 로그아웃 후 cookie 잔존 방어
    token_hash = hashlib.sha256(charisg_session.encode()).hexdigest()
    with get_db() as conn:
        row = conn.execute(
