...
from backend.purchase.services.coupang_service import get_seller_product, coupang_account
...
def sync_one(channel_product_id: str, account: str = "old") -> Optional[dict]:
    """단일 셀러 상품 status 조회 + DB 업데이트. account='old'|'new' 로 계정 라우팅."""
...
def sync_all(only_unchecked_days: int | None = None,
    ...
    with get_db() as conn:
        rows = conn.execute(
            f"SELECT channel_product_id, COALESCE(coupang_account,'old') AS coupang_account "
            f"FROM listings_pa {where}",
            params,
        ).fetchall()
    ...
    for i, r in enumerate(rows, 1):
        ...
        acct = r["coupang_account"] or "old"
        try:
            res = sync_one(cpid, acct)
            if res:
