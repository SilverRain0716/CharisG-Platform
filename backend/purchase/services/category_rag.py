...
{{"mapped_category": "<선택한 ID>", "confidence": 0.0~1.0}}"""
    ...
    try:
        ...
        # 2026-05-19: LLM 이 후보에 없는 ID 반환한 케이스 — silent fallback 가시성
        logger.warning(
            f"resolve_category: LLM 이 후보 외 ID 반환 — "
            f"product={product_name!r} mc={mc!r} valid={len(valid_ids)}개 → embedding fallback"
        )
    except (json.JSONDecodeError, TypeError, ValueError) as e:
        # 2026-05-19: LLM JSON parse 실패 silent → 로그로 가시성 확보
        logger.warning(
            f"resolve_category: LLM JSON parse 실패 — product={product_name!r} "
            f"err={e} raw[:80]={(result or '')[:80]!r} → embedding fallback"
        )
