...
def receive_order(
    ...
    # ★2026-08-08: 종전에는 마스터 product_id 로만 캐시를 채워, 옵션 상품 주문의
    #   목록/이미지/ASIN 이 전부 '그룹 대표 형제'로 표시됐다(주문 1730: 퍼플 주문인데
    #   블루로 표시). 실제 팔린 옵션이 있으면 그것을 우선한다 — amazon_prep 의
    #   `child_product_id or product_id` 와 동일한 규칙으로 통일.
    denorm = _lookup_product_denorm(child_product_id or product_id)
