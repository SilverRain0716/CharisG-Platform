...
멀티계정(2026-07-30): NAVER_ACTIVE=old|new + `with naver_account("new"):` 컨텍스트.
  신계정 키는 NAVER_NEW_COMMERCE_CLIENT_ID/SECRET. 토큰 캐시는 계정별 분리.
"""
import base64
import contextvars
import copy
...
from backend_shared._config import NAVER_ACTIVE, naver_cfg
...
# ── 계정 라우팅 (구/신 스마트스토어 멀티계정, 2026-07-30) ──────
# 기본값(context 미설정) = NAVER_ACTIVE(.env, 기본 old) → 기존 동작과 100% 동일.
# `with naver_account("new"):` 블록 안에서만 신계정 자격증명으로 호출.
_ACCOUNT_CTX = contextvars.ContextVar("naver_account", default=None)


@contextmanager
def naver_account(account):
    """이 블록 안의 모든 네이버 커머스 호출을 지정 계정('old'|'new')으로 라우팅."""
...
def active_account() -> str:
    """현재 컨텍스트의 활성 계정 ('old'|'new'). 미설정 시 .env NAVER_ACTIVE."""
    return _ACCOUNT_CTX.get() or NAVER_ACTIVE


def _client_id() -> str:
    return naver_cfg("CLIENT_ID", active_account())


def _client_secret() -> str:
    return naver_cfg("CLIENT_SECRET", active_account())


# ── Token Cache ───────────────────────────────────────────────
# ★계정별 분리 필수 — 단일 캐시면 다른 계정 토큰으로 호출된다.
_TOKEN_LOCK = threading.Lock()
_TOKEN_CACHE: dict = {}   # account -> {"access_token":..., "expires_at":...}
...
def _get_token() -> Optional[str]:
    ...
    if not (_client_id() and _client_secret()):
        logger.warning("NAVER 커머스 자격증명 미설정 (account=%s)", acct)
        return None

    now = time.time()
    slot = _TOKEN_CACHE.get(acct) or {}
    cached = slot.get("access_token")
    expires_at = slot.get("expires_at", 0.0)
    ...
    with _TOKEN_LOCK:
        ...
        if not issued:
            return None
        tok, exp = issued
        _TOKEN_CACHE[acct] = {"access_token": tok, "expires_at": exp}
        return tok
...
def get_changed_product_orders(
    ...
    last_changed_type (2026-05-10 기준 valid):
        PAY_WAITING / PAYED / DISPATCHED / PURCHASE_DECIDED / EXCHANGE_OPTION /
        GIFT_RECEIVED / CLAIM_REJECTED / COLLECT_DONE / CLAIM_REQUESTED /
        CLAIM_HOLDBACK / HOPE_DELIVERY_INFO_CHANGED
    deprecated (400 응답): CANCELED, RETURNED, EXCHANGED, CANCELED_BY_NOPAYMENT,
        ADMIN_CANCELING — CLAIM_* 통합 흐름으로 대체됨
    """
    token = _get_token()
    if not token:
        return []
    ...
    out: list[str] = []
    next_seq: Optional[str] = None  # 페이지네이션 커서 (응답의 more 블록)
    MAX_PAGES = 20  # 안전 한도
    ...
    for _page in range(MAX_PAGES):
        ...
        if r is None or r.status_code >= 400:
            logger.warning(
                f"네이버 주문 변경ID 조회 실패: "
                f"{r.status_code if r is not None else 'no-response'} {r.text[:200] if r is not None else ''}"
            )
            return out
        body = r.json() or {}
        data = body.get("data") or {}
        statuses = data.get("lastChangeStatuses") or []
        out.extend(s.get("productOrderId") for s in statuses if s.get("productOrderId"))

        # 다음 페이지 커서 — 네이버 응답: data.more.moreSequence
        more = data.get("more") if isinstance(data.get("more"), dict) else {}
        next_seq = more.get("moreSequence") if more else None
        if not next_seq:
            break
    else:
        logger.warning(
            "[naver-changed-orders] 페이지네이션 MAX_PAGES(%d) 도달 — 일부 누락 가능 type=%s window=%s~%s",
            MAX_PAGES, last_changed_type, last_changed_from, last_changed_to,
        )
...
def get_product_order_details(product_order_ids: list[str]) -> list[dict]:
    ...
    for i in range(0, len(product_order_ids), BATCH):
        ...
        if r is None or r.status_code >= 400:
            logger.warning(
                f"네이버 주문 상세 조회 실패: "
                f"{r.status_code if r is not None else 'no-response'} {r.text[:300] if r is not None else ''}"
            )
            continue
        body = r.json() or {}
        data = body.get("data") or []
