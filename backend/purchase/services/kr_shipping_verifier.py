...
실측 처리 시간: 건당 ~25-30초 (Amazon dp 페이지 다운로드 + 정규식 매칭 + 4초 sleep).
amazon_kr_shipping._REQUEST_INTERVAL=4.0 의 sleep 보다 실 HTML 처리 시간이 더 큼.
"""
...
# 동시 가격 sync 등 다른 cold.db writer 와 충돌 시 busy_timeout(180s) 후
# OperationalError("database is locked") 발생. 검증 결과(특히 eligible)
# 손실 방지를 위해 짧은 backoff retry.
_LOCK_RETRY_BACKOFFS = (5, 15, 30)  # seconds — 합 50초, busy_timeout 합쳐 ~10분 wait 가능
...
def _select_targets(
    ...
    # 2026-05-15: pre-upload 호출 지원 — status='pending' 도 검증 대상.
    sql = """SELECT p.asin AS asin,
                    MIN(lp.kr_shipping_checked_at) AS oldest_check
               FROM listings_pa lp
               JOIN products p ON p.id = lp.product_id
              WHERE lp.status IN ('pending','listed') AND p.asin IS NOT NULL"""
    params: list = []
    if channel:
        sql += " AND lp.channel = ?"
...
def _save_result(listing_id: Optional[int], asin: str, eligible: bool) -> int:
    ...
    busy_timeout 으로도 락 회피 못한 경우 _LOCK_RETRY_BACKOFFS 만큼 재시도.

    반환: 영향 받은 row 수. 락 retry 모두 실패 시 OperationalError 그대로 raise.
    """
    now = _now_iso()
    attempts = (0,) + _LOCK_RETRY_BACKOFFS
    for i, backoff in enumerate(attempts):
        ...
        try:
            ...
            if "locked" not in str(e).lower() or i == len(attempts) - 1:
                raise
            logger.warning(
                "[kr-shipping-verifier] %s save 락 충돌 (attempt %d/%d) — %ds 후 재시도",
                asin, i + 1, len(attempts), attempts[i + 1] if i + 1 < len(attempts) else 0,
            )
