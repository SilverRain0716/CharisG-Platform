...
HEADER_ALIASES = {
    ...
    "price_usd":     ["가격($)", "가격", "price", "price_usd", "가격(USD)", "Price ($)", "Price (USD)", "Price"],
...
def discover_tabs(sheet_id: str) -> tuple[list[dict], bool]:
    ...
    Returns:
        (tabs, fallback_used) — fallback_used=True 이면 정규식 매칭 실패해서
        gid=0 추측한 케이스. 사용자에게 다중 탭 누락 가능성 알릴 용도.
    """
    ...
    fallback_used = False
    if not tabs:
        # P2 (2026-05-19): silent fallback 위험 — 다중 탭 시트인데 1개로 인식될 수 있음.
        # caller 가 사용자 알림 띄울 수 있도록 fallback_used 플래그 동반 반환.
        logger.warning("discover_tabs: no tabs matched — fallback to gid=0 (다중 탭 누락 가능성)")
        tabs = [{"name": "Sheet1", "gid": 0}]
        fallback_used = True
    return tabs, fallback_used
...
def fetch_tab_csv(sheet_id: str, gid: int) -> list[dict]:
    ...
    if r.status_code != 200:
        # P3 (2026-05-19): silent return [] 금지 — 사유 추적 가능하도록 명시 로그
        logger.warning(
            f"fetch_tab_csv: gid={gid} status={r.status_code} url={url}"
        )
        return []
...
def import_from_sheet_url(sheet_url: str) -> dict:
    ...
    try:
        tabs, fallback_used = discover_tabs(sheet_id)
    ...
    for tab in tabs:
        ...
        for r in rows:
            ...
            if p is None:
                continue
            # P1 (2026-05-19): price 둘 다 None 인 행은 INSERT 거부 (cost=0 사고 줄기 차단).
            # sourcing_promote 단계의 가드가 또 한 번 잡지만 import 단계 가시성 위해 여기서 카운트.
            if p["price_usd"] is None and p["price_krw"] is None:
                price_missing += 1
                logger.warning(
                    f"parse_row: price_missing — gid={tab['gid']} asin={p['asin']} "
        ...
        total_price_missing += price_missing
        result_tabs.append({
            "name": tab["name"],
            "gid": tab["gid"],
            "imported": imported,
    ...
    return {
        ...
        "total_price_missing": total_price_missing,
        "fallback_tabs": fallback_used,  # P2: 다중 탭 누락 가능성 알림
    }
