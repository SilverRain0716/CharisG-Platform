...
async def _process_sheet(item: dict) -> None:
    ...
    try:
        ...
        price_missing = r.get("total_price_missing", 0)
        fallback_tabs = r.get("fallback_tabs", False)

        # 2026-05-19: cost=0 사고 후 — import 단계 가시성 강화
        msg_parts = [f"import 완료 — {imported}건"]
        if price_missing:
            msg_parts.append(f"가격누락 {price_missing}건 제외")
        if fallback_tabs:
            msg_parts.append("⚠️ 탭 자동인식 실패 — gid=0 만 처리됨 (다중 탭 시트면 누락 가능)")
        _update(sid, imported=imported, current_step=" · ".join(msg_parts))
    ...
    try:
        ...
        from backend.purchase.services.job_lock import async_shared_unit
        promote_job = create_promote_job(imported)
        # ★ turnstile SHARED — 드레인과 동시 실행(짝꿍). 2026-06-10: 단계 통째 보유 →
        #   청크 양보(yield_unit)로 변경, 긴 promote 가 타임잡(writer) 굶기던 문제 봉합.
        _promote_unit = async_shared_unit("sheet:promote")
        async with _promote_unit:
            await run_promote_background(promote_job, yield_unit=_promote_unit)
    ...
    try:
        ...
        from backend.purchase.services.job_lock import async_shared_unit
        # 2026-06-10: 청크 양보(yield_unit) — 긴 detailing 이 타임잡(writer) 굶기던 문제 봉합.
        _detail_unit = async_shared_unit("sheet:detailing")
        async with _detail_unit:
            await run_two_stage_batch(detail_job_id, new_pids, yield_unit=_detail_unit)
    ...
    # ── 3b. 상세 인포그래픽 prewarm (detailing 단계에서 1회 생성·캐시) ──
    # build_payload(리스트 단계)는 캐시만 참조 → relist 시 Gemini 재호출/이미지 변동 방지.
    # 생성 실패는 리스팅을 막지 않음(build_payload 가 lazy 폴백 생성).
    try:
        from backend.purchase.services.detail_infographic import render_infographic
        _ig_ok = 0
        for _pid in new_pids:
            try:
                if await asyncio.to_thread(render_infographic, _pid, True):
                    _ig_ok += 1
            except Exception as _ie:
                logger.warning(f"[sheet-queue] sid={sid} pid={_pid} 인포그래픽 prewarm 실패: {_ie}")
        _update(sid, current_step=f"인포그래픽 생성 {_ig_ok}/{len(new_pids)}")
    except Exception:
        logger.exception(f"[sheet-queue] sid={sid} 인포그래픽 prewarm 단계 오류(계속)")

    # ── 4. 채널 보내기 (3-케이스 라우팅) ─────────────────
    # 2026-06-03 재설계: 수집 ASIN 3케이스를 parent_asin 컬럼으로 라우팅.
    #   단독(parent_asin NULL) → 쿠팡 단일 + 네이버 단일
    #   변형(parent_asin 보유) → 쿠팡은 group_registration_queue(멀티옵션, group-worker 비동기)
    #                            네이버는 단일 (별도 지시 전까지 — 사용자 결정)
    # 변형의 누락 자식 수집·멀티옵션 등록은 group_registration_worker 가 전담.
    channels_csv = item.get("target_channels") or "coupang"  # 2026-06-08 네이버 중지
    channels = [c.strip() for c in channels_csv.split(",") if c.strip()]
    want_coupang = "coupang" in channels
    want_smartstore = "smartstore" in channels
    _update(sid, status="channelsending",
            current_step="채널 보내기 중 (3-케이스 라우팅)")

    # parent_asin 으로 단독/변형 분류
    with get_db() as conn:
        ph = ",".join("?" * len(new_pids))
        prows = conn.execute(
            f"SELECT id, parent_asin FROM products WHERE id IN ({ph})", new_pids
        ).fetchall()
    variation_pids = {r["id"] for r in prows if r["parent_asin"]}
    family_parents = sorted({r["parent_asin"] for r in prows if r["parent_asin"]})
    ...
    try:
        ...
        for pid in new_pids:
            chs = []
            if want_smartstore:
                chs.append("smartstore")              # 네이버: 변형도 단일
            if want_coupang and pid not in variation_pids:
                chs.append("coupang")                 # 쿠팡: 단독만 단일
            if not chs:
                continue
            try:
                await asyncio.to_thread(send_to_channels, pid, chs)
                sent_single += 1
            except Exception as e:
                logger.warning(f"[sheet-queue] sid={sid} pid={pid} send 실패: {e}")

        # 쿠팡: 변형 패밀리 → group_registration_queue (멀티옵션)
        queued_groups = 0
        if want_coupang and family_parents:
            sheet_ref = f"sheet_queue:{sid}"
            with get_db() as conn:
                for pa in family_parents:
                    dup = conn.execute(
                    ...
                    conn.execute(
                        """INSERT INTO group_registration_queue
                           ...
                           VALUES (?, ?, 'queued', 1, 'coupang', ?)""",
                        (pa, sheet_ref, _now()),
                    )
                    queued_groups += 1
                conn.commit()

        _update(sid, current_step=(
            f"채널 보내기 완료 — 단일 {sent_single} "
            f"(쿠팡단독+네이버), 쿠팡그룹큐 {queued_groups}패밀리"))
    except Exception as e:
    ...
    try:
        from backend.purchase.services.job_lock import async_shared_unit
        async with async_shared_unit("sheet:upload"):
