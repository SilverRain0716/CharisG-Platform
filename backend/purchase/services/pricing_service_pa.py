...
# 2026-08-12: 11번가·G마켓·옥션 수수료율 13% 적용.
#   ★_FEE_KEY 는 margin_calculator.py 에도 같은 맵이 있다. 한쪽만 고치면 조용히 어긋난다.
#   여기 없는 채널은 raise 라서, 채널을 늘릴 때 반드시 settings 행도 같이 넣어야 한다
#   (없으면 _get_setting_float 이 "settings 에 X 누락" 으로 raise).
_ALLOWED_CHANNELS = ("smartstore", "coupang", "elevenst", "gmarket", "auction")
_FEE_KEY = {
    "smartstore": "smartstore_fee_rate",
    "coupang": "coupang_fee_rate",
    "elevenst": "elevenst_fee_rate",
    "gmarket": "gmarket_fee_rate",
    "auction": "auction_fee_rate",
}
...
def calculate_sale_krw(
    ...
    if cost_usd <= 0:
        # 2026-05-19 사고: cost_usd=0 인 product 가 sourcing 단계를 우회해
        # 들어오면 sale_krw 가 fixed cost 만으로 산출돼 마진 0 listing 발생.
        # 가격 계산은 cost>0 일 때만 의미가 있으므로 raise.
        raise ValueError(f"cost_usd 가 0 이하면 가격 산출 불가: cost_usd={cost_usd}")
    if amazon_shipping_usd < 0 or cj_shipping_usd < 0:
        raise ValueError("shipping 값은 0 이상이어야 함")
    ...
    # ★절대마진 상한(2026-07-25): 순마진 > 상한이면 판매가를 낮춰 상한으로 고정.
    #   설정 margin.max_net_margin_krw (0/미설정=무제한). 고가·고마진 상품 경쟁력 확보용.
    #   목표마진율(%) 대신 절대 상한액으로 역산 → 그만큼 저렴해짐, 마진은 상한 보장.
    _max_margin = _get_setting_float("margin.max_net_margin_krw", default=0.0)
    if _max_margin > 0 and net_margin_krw > _max_margin:
        _denom_cap = 1.0 - fee_rate - return_reserve_pct
        if _denom_cap > 0:
            sale_krw = int(round((_max_margin + cost_krw_var + cost_krw_fix) / _denom_cap / 100) * 100)
            fee_krw = sale_krw * fee_rate
            return_krw = sale_krw * return_reserve_pct
            net_margin_krw = (
                sale_krw - cost_krw_var - fee_krw - return_krw
