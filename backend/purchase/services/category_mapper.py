...
def _gemini_rotate(prompt: str, json_mode: bool = False, timeout: int = 20, rounds: int = 3):
    """카테고리용 Gemini — 키 로테이션 fast-fail(FALLBACK/_5/default/_2-4). 한 라운드(6키)
    전부 503/429면 짧은 backoff(3s) 후 재시도(최대 rounds회) — Gemini flapping 흡수. 첫 200 text, 전부실패 None."""
    import time as _t
    import os as _o0
    if _o0.environ.get("PA_SKIP_GEMINI") == "1":
        return None
    cfg = {"contents": [{"parts": [{"text": prompt}]}]}
    if json_mode:
    ...
    keys = []
    for n in ("GEMINI_API_KEY_FALLBACK", "GEMINI_API_KEY_5", "GEMINI_API_KEY",
    ...
    for rnd in range(rounds):
        for k in keys:
            try:
                r = requests.post(base + k, json=cfg, timeout=timeout)
...
def _ai_suggest_keywords(product_name: str) -> list[str]:
    ...
    try:
        text = _gemini_rotate(prompt, timeout=15)
        if not text:
            return []
...
def find_coupang_category_with_gemini(
    ...
    try:
        text = _gemini_rotate(prompt, json_mode=True, timeout=20)
        if not text:
            return {"code": None, "name": "", "path": "", "score": 0, "needs_review": True}
        try:
...
# ── 11번가 ────────────────────────────────────────────────────
# 쿠팡·네이버와 같은 패턴. 다른 점 두 가지:
#   1) 후보를 elevenst_categories 에서 뽑되 **is_leaf=1 AND global_dlv=1** 로 좁힌다.
#      해외배송 불가 카테고리에 구매대행 상품을 올리면 등록 자체가 거부된다.
#   2) 카테고리 번호가 문자열(dispNo)이다. 쿠팡처럼 int 로 캐스팅하면 앞자리 0 이 날아간다.
ELEVENST_REVIEW_THRESHOLD = 70


def _search_elevenst_candidates(terms: list[str], limit_per_term: int = 8) -> list[dict]:
    """elevenst_categories 에서 leaf + 해외배송 가능 후보 검색."""
    from backend.purchase.database import get_db
    candidates: list[dict] = []
    seen: set[str] = set()
    with get_db() as conn:
            ...
            rows = conn.execute(
                """SELECT disp_no, name, full_path FROM elevenst_categories
                   WHERE (name LIKE ? OR full_path LIKE ?)
                     AND is_leaf = 1 AND global_dlv = 1
                   LIMIT ?""",
                (f"%{term}%", f"%{term}%", limit_per_term),
            ).fetchall()
            for r in rows:
...
def find_elevenst_category_with_gemini(
    product_name: str, category_hint: str = "",
    features: list = None,
    sample_titles: list = None, sample_en: str = "",
) -> dict:
    """11번가 leaf 카테고리 자동 매핑.

    반환: {"code": str|None, "name": str, "path": str, "score": int,
    ...
    if category_hint:
        terms = [category_hint] + terms
    terms = terms[:8]
    candidates = _search_elevenst_candidates(terms, limit_per_term=8)

    if len(candidates) < 10:
        suggested = _ai_suggest_keywords(name_for_search or product_name)
        if suggested:
            extra = _search_elevenst_candidates(suggested, limit_per_term=6)
            existing = {c["code"] for c in candidates}
            for c in extra:
                if c["code"] not in existing:
                    candidates.append(c)
    ...
    if not candidates:
        logger.warning(f"[11st-cat] 후보 0건: {(product_name or '')[:50]}")
        return {"code": None, "name": "", "path": "", "score": 0, "needs_review": True}
    ...
    prompt = (
        f"다음 상품에 가장 적합한 11번가 leaf 카테고리를 선택하세요.\n\n"
        f"상품명(원본): {product_name}\n"
        f"상품명(한국어): {name_for_search}\n"
        f"카테고리 힌트: {category_hint}\n"
        f"상품 특징: {features_text}{samples_block}\n\n"
        f"후보 카테고리:\n{candidate_text}\n\n"
        f"규칙:\n"
        f"1. 반드시 위 후보 중 하나만 선택 (코드는 문자열 그대로)\n"
        f"2. 같은 단어가 여러 카테고리에 있을 때 샘플 제목과 영문 원문을 종합해 "
        f"의도 파악 — 모호하면 score 낮춰서 답변\n"
        f"3. 매칭 신뢰도 점수(0~100) 함께 답변 (90+ 정확, 70-89 적당, 50-69 애매, 0-49 추측)\n"
        f"4. JSON 형식으로만 응답:\n"
        f'   {{"code": "1008491", "score": 85, "reason": "한 줄 이유"}}'
    )
    ...
    try:
        ...
        try:
            ...
            digits = "".join(c for c in text if c.isdigit())
            parsed = {"code": digits or None, "score": 50}
        code = parsed.get("code")
        code = str(code).strip() if code is not None else None
        score = int(parsed.get("score") or 0)
        matched = next((c for c in candidates if c["code"] == code), None)
        if not matched:
            logger.warning(f"[11st-cat] Gemini 후보 외 응답: code={code}")
            return {"code": None, "name": "", "path": "", "score": 0, "needs_review": True,
                    "raw_response": text[:200]}
        needs_review = score < ELEVENST_REVIEW_THRESHOLD
        logger.info(f"[11st-cat] {(product_name or '')[:30]} → {matched['path']} "
                    f"(score={score}, review={needs_review})")
        return {
            "code": matched["code"],
            "name": matched["name"],
        ...
        logger.warning(f"[11st-cat] Gemini 매핑 실패: {e}")
        return {"code": None, "name": "", "path": "", "score": 0, "needs_review": True}
