...
def catalog_batch(asins: list, included_data: list, marketplace: str = "US") -> dict:
    """searchCatalogItems로 최대 20개씩 조회 → {asin: item}. item은 getCatalogItem payload와 동일구조(검증됨).
    누락 ASIN은 dict에 없음(호출측 단건 폴백). 2 RPS(청크간 sleep 0.55). (2026-07-25 배치발굴)"""
    out = {}
    if not asins:
        return out
    try:
        ...
        logger.warning(f"[catalog-batch] sp_api 모듈 없음: {e}")
        return out
    import time as _t
    mp_id = {"US": "ATVPDKIKX0DER", "CA": "A2EUQ1WTGCTBG2", "MX": "A1AM78C64UM0Y8"}.get(marketplace, "ATVPDKIKX0DER")
    uniq = list(dict.fromkeys(a for a in asins if a))
    try:
        creds = get_credentials()
        cat = CatalogItems(credentials=creds, marketplace=getattr(Marketplaces, marketplace, Marketplaces.US), version="2022-04-01")
        for i in range(0, len(uniq), 20):
            chunk = uniq[i:i + 20]
            try:
                res = cat.search_catalog_items(identifiers=",".join(chunk), identifiersType="ASIN",
                                               includedData=included_data, marketplaceIds=[mp_id], pageSize=20)
                for it in ((res.payload or {}).get("items") or []):
                    a = it.get("asin")
                    if a:
                        out[a] = it
            except Exception as e:
                logger.warning(f"[catalog-batch] chunk 실패({len(chunk)}): {e}")
            if i + 20 < len(uniq):
                _t.sleep(0.55)
    except Exception as e:
        logger.warning(f"[catalog-batch] 실패: {e}")
    return out
...
def discover_group(parent_asin: str, marketplace: str = "US") -> Optional[dict]:
    ...
    if not meta:
        return None
    # 단독 ASIN 가드 — variation_theme 도 child_asins 도 없으면 single product.
    # variation_groups 에 false-positive row 만들지 않고 caller 에는 meta 만 반환
    # (worker 가 단독 fallback path 로 자연스럽게 분기).
    if not meta.get("variation_theme") and not (meta.get("child_asins") or []):
        logger.info(f"[group-discovery] {parent_asin} 단독 ASIN — variation_groups INSERT skip")
        return meta
    try:
        _persist_group(meta)
    except Exception as e:
