...
def _map_naver_order_to_row(entry: dict, product_id_by_origin: dict) -> Optional[dict]:
    ...
    # ★폴백(2026-08-08) — 쿠팡과 동일. 네이버는 optionManageCode 에 자식 ASIN 이 들어온다.
    if child_product_id is None:
        from backend.purchase.services.order_child_resolve import resolve_child_by_sku
        child_product_id, child_asin = resolve_child_by_sku(
            _pick(po, "optionManageCode", "sellerProductCode"), "smartstore")

    # ★2차 폴백 — 옵션명 매칭. 가격 분산이 커서 조합형으로 담지 못한 상품군
    #   (묶음수량·크기 변형)은 단독형이라 ASIN 이 실려오지 않는다. 그런 건은
    #   옵션명이 유일한 단서다 — 등록 시 형제끼리 겹치지 않게 유니크화해 뒀다.
    if child_product_id is None:
        opt_nm = (_pick(po, "productOption", "optionName", "optionCode") or "").strip()
        if opt_nm and origin_no:
            try:
                from backend.purchase.database import get_db
                with get_db() as conn:
                    row = conn.execute(
                        """SELECT lo.child_product_id, p.asin
                           ...
                           LEFT JOIN products p ON p.id = lo.child_product_id
                           WHERE l.channel='smartstore' AND l.channel_product_id=?
                             AND lo.channel_option_name = ? LIMIT 1""",
                        (str(origin_no), opt_nm),
                    ).fetchone()
                if row and row["child_product_id"]:
                    child_product_id = row["child_product_id"]
                    child_asin = row["asin"]
                    logger.info(f"[smartstore-order-map] 옵션명 매칭 성공 {opt_nm!r} "
                                f"→ child pid={child_product_id}")
                else:
                    logger.warning(f"[smartstore-order-map] 옵션명 {opt_nm!r} 매칭 실패 "
                                   f"(origin={origin_no}) — 대표 상품으로 폴백된다")
            except Exception as e:
                logger.warning(f"[smartstore-order-map] 옵션명 매칭 오류: {e}")
