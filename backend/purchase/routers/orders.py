...
def kanban(
    ...
    account: Optional[str] = None,   # 쿠팡 계정 필터: 'old'|'new' (None=전체)
):
    ...
    if account in ("old", "new"):
        where.append("coupang_account=?")
        params.append(account)
    where_sql = ("WHERE " + " AND ".join(where)) if where else ""
    with get_db_hot() as conn:
        rows = conn.execute(
            f"""SELECT id, channel, channel_order_id, customer_name, sale_price_krw,
                       current_step, placed_at, product_name_cache, coupang_account FROM orders
                       {where_sql} ORDER BY placed_at DESC LIMIT 200""",
            tuple(params),
        ).fetchall()
    for r in rows:
...
def list_orders(
    ...
    channel: Optional[str] = None,
    account: Optional[str] = None,   # 쿠팡 계정 필터: 'old'|'new'
    limit: int = 100,
):
    where = []
    params = []
...
def advance(oid: int, body: AdvanceBody, user: dict = Depends(current_user)):
    # amazon_purchase 차단 제거 (2026-05-13) — 사용자가 단계만 advance 한 후
    # 별도로 발주번호 입력하는 흐름도 허용. amazon_order_id NULL 주문은 대시보드의
    # amazon_purchase_no_id 카운터로 후속 인지 (정식 경로 /amazon-order 도 그대로 사용 가능).
...
# ──────────────────────────────────────────────
# 문자 발송 (수동) — 자동 발송 없음. 운영자가 본문을 직접 작성해 보낸다.
# ──────────────────────────────────────────────

class SmsBody(BaseModel):
    number_type: Literal["safe", "real"] = "safe"
    text: str
    reason: Optional[str] = None   # number_type='real' 일 때 필수


@router.get("/sms/balance")
def sms_balance(user: dict = Depends(current_user)):
    """솔라피 잔액. balance(캐시) + point(무료 포인트) 둘 다 발송에 쓰인다."""
    from backend.purchase.services import sms_service
    try:
...
@router.get("/{oid}/sms")
def sms_history(oid: int, user: dict = Depends(current_user)):
    from backend.purchase.services import sms_service
    return sms_service.list_order_sms(oid)


@router.post("/{oid}/sms")
def send_sms(oid: int, body: SmsBody, user: dict = Depends(current_user)):
    """주문 상세에서 수동 발송.

    수신번호는 서버가 주문 레코드에서 직접 꺼낸다 — 클라이언트가 임의 번호를
    넘기지 못하게 해서 엉뚱한 사람에게 발송되는 사고를 막는다.
    """
    from backend.purchase.services import sms_service

    with get_db_hot() as conn:
        row = conn.execute(
            "SELECT customer_phone, orderer_real_phone FROM orders WHERE id=?", (oid,)
        ).fetchone()
    if not row:
        raise HTTPException(404, "주문 없음")

    to_raw = row["customer_phone"] if body.number_type == "safe" else row["orderer_real_phone"]
    if not to_raw:
        label = "안심번호" if body.number_type == "safe" else "실번호"
        raise HTTPException(422, f"이 주문에는 {label}가 없습니다")

    try:
        return sms_service.send_order_sms(
            order_id=oid,
            to_raw=to_raw,
