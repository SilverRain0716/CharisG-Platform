...
# 계정 구분(2026-08-10) — naver_account: 'new'=카리스 글로벌, 그 외/NULL=카리스G(구).
# 초기 데이터에 NULL 이 섞여 있어 COALESCE 로 'old' 취급한다.
_ACCT = "COALESCE(NULLIF(l.naver_account,''),'old')"
...
def list_listings(
    ...
    account: str | None = None,   # 계정 필터: 'old'(카리스G)|'new'(카리스 글로벌), None=전체
    limit: int = 50,           # 서버사이드 keyset 페이징 (2026-06-02)
    before_id: int | None = None,  # 커서: id 미만(다음 페이지). 깊은 페이지 O(1).
    offset: int = 0,           # (하위호환 — before_id 미지정 시)
):
    base_where = ["l.channel='smartstore'"]
    base_params: list = []
    if status:
        base_where.append("l.status=?"); base_params.append(status)
    if account in ("old", "new"):
        base_where.append(f"{_ACCT}=?"); base_params.append(account)
    ...
    with get_db() as conn:
        ...
        # 계정별 카운트 — ★현재 상태 탭 기준. 탭 숫자와 필터 버튼 숫자가 어긋나면
        # 사용자가 어느 쪽을 믿어야 할지 알 수 없으므로 같은 조건으로 센다.
        acct_where = ["l.channel='smartstore'"]
        acct_params: list = []
        if status:
            acct_where.append("l.status=?"); acct_params.append(status)
        acct_rows = conn.execute(
            f"SELECT {_ACCT} a, COUNT(*) c FROM listings_pa l "
            f"WHERE {' AND '.join(acct_where)} GROUP BY a",
            tuple(acct_params),
        ).fetchall()
...
async def _run_upload_background(job_id: str, product_ids: list[int], channel: str):
    ...
    if product_ids:
        ...
        if already_listed:
            logger.info(
                f"[{channel}-upload] 사전 dedup: 이미 listed {len(already_listed)}건 skip"
            )
            product_ids = [pid for pid in product_ids if pid not in already_listed]
            counters["skipped"] += len(already_listed)
...
_SS_COUNT_CACHE_KEY = "smartstore.product_count_cache_v2"
_SS_COUNT_TTL_SEC = 3600


@router.get("/products/count")
def products_count(refresh: bool = False, user: dict = Depends(current_user)):
    """네이버 측 SALE 상태 상품 수 — 계정(구/신)별로 각각 조회.

    ★한도 1,000 은 **계정마다 따로** 걸린다. 합계만 보면 어느 계정이 찬 건지
      알 수 없어 신규 등록이 왜 막히는지 진단이 안 된다. 그래서 구계정(카리스G)·
      신계정(카리스 글로벌)을 각각 조회해 나눠 돌려준다.

    호출 2회로 끝나므로 빠르나 일관성을 위해 settings 1시간 캐시.
    refresh=true 면 캐시 무시하고 강제 재조회.
    """
    import json
    from backend.purchase.services.naver_commerce_service import (
        get_sale_product_count, naver_account)
    ...
    # 계정별 조회 — 한쪽이 실패해도 다른 쪽 숫자는 살린다(None 으로 구분).
    by_account: dict = {}
    for acct in ("old", "new"):
        try:
            with naver_account(acct):
                by_account[acct] = get_sale_product_count()
        except Exception as e:  # noqa: BLE001
            logger.warning(f"네이버 SALE 카운트 조회 실패({acct}): {e}")
            by_account[acct] = None
    known = [v for v in by_account.values() if v is not None]
    total = sum(known) if known else None
    result = {
        "total": total,
        "by_account": by_account,
        "limit_per_account": 1000,
