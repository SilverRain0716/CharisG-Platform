...
from backend.purchase.services.coupang_service import get_orders, sync_orders, coupang_account
...
def list_listings(
    ...
    account: str | None = None,   # 계정 필터: 'old'|'new' (None=전체)
    limit: int = 50,           # 서버사이드 keyset 페이징 (2026-06-02)
    before_id: int | None = None,  # 커서: 이 id 미만(=다음 페이지). idx_listings_pa_channel_id 로 깊은 페이지도 O(1).
    offset: int = 0,           # (하위호환 — before_id 미지정 시에만 사용)
):
    # 탭(채널+상태) 기준 — 카운트용
    base_where = ["l.channel='coupang'"]
    base_params: list = []
    if status:
        base_where.append("l.status=?"); base_params.append(status)
    if account in ("old", "new"):
        base_where.append("l.coupang_account=?"); base_params.append(account)
    base_sql = " AND ".join(base_where)
    # 행 조회 — keyset(id < before_id) 우선, 없으면 offset
    row_where = list(base_where); row_params = list(base_params)
    use_offset = before_id is None
    if before_id is not None and before_id > 0:
        row_where.append("l.id < ?"); row_params.append(before_id)
    row_sql = " AND ".join(row_where)
    with get_db() as conn:
        rows = conn.execute(
            f"""SELECT l.*, p.title_ko, p.title_en, p.asin
               FROM listings_pa l JOIN products p ON l.product_id = p.id
               WHERE {row_sql}
               ORDER BY l.id DESC LIMIT ?{' OFFSET ?' if use_offset else ''}""",
            (*row_params, limit, offset) if use_offset else (*row_params, limit),
        ).fetchall()
        # 현재 필터(탭) 기준 전체 건수 — 페이지 수 표시용 (before_id 무관)
        filtered_total = conn.execute(
            f"SELECT COUNT(*) c FROM listings_pa l WHERE {base_sql}", tuple(base_params),
        ).fetchone()["c"]
        ...
        # 속성보정 탭 뱃지: MANDATORY 사유로 excluded 된 건
        excluded_mandatory = conn.execute(
            "SELECT COUNT(*) c FROM listings_pa WHERE channel='coupang' AND status='excluded' "
            "AND error_message LIKE 'MANDATORY%'"
        ).fetchone()["c"]
        # 계정별 카운트 (구/신 구분 필터 뱃지용) — listed 기준
        acct_rows = conn.execute(
            "SELECT COALESCE(coupang_account,'old') a, COUNT(*) c FROM listings_pa "
            "WHERE channel='coupang' AND status='listed' GROUP BY a"
        ).fetchall()
        by_account = {r["a"]: r["c"] for r in acct_rows}
    return {
        "items": [dict(r) for r in rows],
        "totals": {"pending": total_pending, "listed": total_listed, "excluded": excluded_mandatory},
        "by_account": {"old": by_account.get("old", 0), "new": by_account.get("new", 0)},
...
async def _run_coupang_upload_bg(job_id: str, product_ids: list[int]):
    ...
    # ── 사전 중복 검사: 이미 listed (channel_product_id 있음) 인 것 skip ──
    # 2026-05-21: 같은 product 재업로드 차단 (네이버 클린프로그램 + 쿠팡 중복 정책)
    if product_ids:
        with get_db() as conn:
            ph = ",".join("?" * len(product_ids))
            already_listed = {r["product_id"] for r in conn.execute(
                f"""SELECT product_id FROM listings_pa
                     WHERE channel='coupang' AND product_id IN ({ph})
                       AND status='listed'
        ...
        if already_listed:
            logger.info(
                f"[coupang-upload-all] 사전 dedup: 이미 listed {len(already_listed)}건 skip"
            )
            product_ids = [pid for pid in product_ids if pid not in already_listed]
            skipped += len(already_listed)
...
def fetch_orders(start: str, end: str, user: dict = Depends(current_user)):
    """라이브 주문조회 — 구·신 두 계정 모두 조회 후 계정 태그하여 합침."""
    out = []
    for acct in ("old", "new"):
        with coupang_account(acct):
            for o in (get_orders(start, end) or []):
                if isinstance(o, dict):
                    o = {**o, "coupang_account": acct}
                out.append(o)
    return {"orders": out}
...
def _tag_orders_account(order_ids, account: str) -> None:
    """방금 INSERT 된 우리 order.id 들의 coupang_account 태깅 ('old'는 기본값이라 skip)."""
    ids = [i for i in (order_ids or [])]
    if not ids or account == "old":
        return
    try:
        ...
        logger.warning(f"[coupang-sync] 계정 태깅 실패 account={account}: {e}")


@router.post("/orders/sync")
def sync_orders_now(days: int = 2, user: dict = Depends(current_user)):
    """수동 동기화 트리거 — 구·신 두 계정의 최근 N일 ACCEPT 주문을 upsert + 계정 태깅.
    ...
    agg = {"fetched": 0, "inserted": 0, "duplicated": 0, "unmapped": 0, "errors": 0}
    per_account = {}
    for acct in ("old", "new"):
        with coupang_account(acct):
            r = sync_orders(start, end)
        _tag_orders_account(r.get("new_order_ids"), acct)
        per_account[acct] = {k: r.get(k, 0) for k in agg}
        for k in agg:
            agg[k] += r.get(k, 0)
    ...
    job_id = uuid.uuid4().hex[:12]
    msg = (
        f"수동 동기화[구+신] [{start} ~ {end}] — 조회 {agg['fetched']}, "
        f"신규 {agg['inserted']}, 중복 {agg['duplicated']}, "
        f"매핑실패 {agg['unmapped']}, 에러 {agg['errors']} "
        f"(구 신규 {per_account['old']['inserted']} / 신 신규 {per_account['new']['inserted']})"
    )
    with get_db() as conn:
        conn.execute(
    ...
    return {"job_id": job_id, "range": {"start": start, "end": end}, "by_account": per_account, **agg}
