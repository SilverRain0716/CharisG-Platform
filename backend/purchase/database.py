...
_BUSY_TIMEOUT_MS = 180000  # 180s — lock contention 회피 (배치+폴러 동시 쓰기 시 30초 부족 — 2026-05-07)
...
def init_db() -> None:
    ...
    cold_runner.apply_all([
        ...
        (36, migrations_dir / "schema_pa_v36.sql", "streaming pipeline phase 1: pipeline_items + pipeline_metrics + views"),
        (38, migrations_dir / "schema_pa_v38.sql", "group_registration_queue (부모 단위 그룹 등록 직렬 워커 큐)"),
        (39, migrations_dir / "schema_pa_v39.sql", "coupang settlement: coupang_settlement(월별 지급) + coupang_revenue(주문별 매출)"),
        (40, migrations_dir / "schema_pa_v40.sql", "naver settlement: naver_settlement(일별 지급) + naver_revenue(건별 정산)"),
    ])
    ...
    hot_runner.apply_all([
        ...
        (3, migrations_dir / "schema_pa_hot_v3.sql", "order_sms: 주문 상세 수동 문자 발송 이력"),
    ])
