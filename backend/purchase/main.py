...
from backend.purchase.routers import (
    ...
    groups, category_mapping, kr_shipping, forwarder_pricing, automation, settlement, ip_screening,
    coupang_new, accounts, listings,
)
from backend.purchase.services import (
    ...
    sheet_queue_worker, stop_korean_mfr_daemon, mfr_classify_daemon,
    keyword_queue_worker, cs_inquiry_poller,
)
...
async def lifespan(app: FastAPI):
    ...
    # 한국 제조사 사후 정리 daemon (30분 cycle)
    korean_mfr_task: asyncio.Task | None = None
    if os.environ.get("PA_DISABLE_KOREAN_MFR_DAEMON") != "1":
        korean_mfr_task = asyncio.create_task(stop_korean_mfr_daemon.run_forever())
        logger.info("한국 제조사 사후 정리 daemon 기동 (interval=%ds)", stop_korean_mfr_daemon.POLL_INTERVAL_SEC)
    # 미분류 brand 자동 분류 daemon (idle 시 chunk 100, 일일 quota 5K)
    # 키워드 큐 워커 (POST /keyword-queue/add 로 적재된 키워드를 일단위 batch 로 처리)
    kw_queue_task: asyncio.Task | None = None
    if os.environ.get("PA_DISABLE_KEYWORD_QUEUE") != "1":
        kw_queue_task = asyncio.create_task(keyword_queue_worker.run_forever())
        logger.info("키워드 큐 워커 기동 (poll=%ds)", keyword_queue_worker.POLL_INTERVAL_SEC)
    mfr_classify_task: asyncio.Task | None = None
    if os.environ.get("PA_DISABLE_MFR_CLASSIFY_DAEMON") != "1":
        mfr_classify_task = asyncio.create_task(mfr_classify_daemon.run_forever())
        logger.info("미분류 brand 분류 daemon 기동 (daily=%d, chunk=%d)",
                    mfr_classify_daemon.DAILY_QUOTA, mfr_classify_daemon.CHUNK_SIZE)
    # 고객문의 폴러 (2026-07-21 신설) — 쿠팡 온라인/콜센터/상품 + 네이버 상품 Q&A 30분 폴링
    cs_inquiry_task: asyncio.Task | None = None
    if os.environ.get("PA_DISABLE_CS_INQUIRY_POLLER") != "1":
        cs_inquiry_task = asyncio.create_task(cs_inquiry_poller.run_forever())
        logger.info("고객문의 폴러 기동 (interval=%ds)", cs_inquiry_poller.POLL_INTERVAL_SEC)
    logger.info("PA API 기동 완료")
    try:
        yield
    finally:
        for t in (poller_task, return_poller_task, smartstore_poller_task, cleanup_task,
                  sheet_worker_task, korean_mfr_task, mfr_classify_task, kw_queue_task,
                  cs_inquiry_task):
            if t:
                t.cancel()
                try:
