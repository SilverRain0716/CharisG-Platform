...
const COLS = [
  ...
  { key: 'notes', label: '특이사항', wrap: true, maxWidth: '220px',
    render: (v) => v ? <span className="text-signal-warn">{v}</span> : '—' },
          ...
          {importMsg && (
            <div
              className={
                ...
                (importMsg.tone === 'err'
                  ? 'bg-soft-err text-signal-err border border-signal-err/40'
                  : 'bg-soft-ok text-signal-ok border border-signal-ok/40')
              }
            >
              {importMsg.text}
            </div>
