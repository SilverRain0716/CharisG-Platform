    ...
    <div className="space-y-4">
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {PRICING_FIELDS.map(({ key, label, unit, step, desc }) => (
          <div key={key} className="rounded-lg border border-ink-200 bg-surface p-4">
            ...
            <div className="flex items-center gap-2">
              <input
                ...
                className="flex-1 rounded-md border border-ink-300 px-3 py-1.5 text-sm focus:border-accent/40 focus:ring-1 focus:ring-indigo-500 outline-none"
              />
      ...
      <div className="flex items-center gap-3">
        ...
        {saveMut.isSuccess && <span className="text-sm text-signal-ok">저장 완료</span>}
          ...
          <Card title="API 연동 상태">
            <div className="grid grid-cols-2 gap-3 lg:grid-cols-3">
              {Object.entries(data.integrations || {}).map(([k, v]) => (
                <div key={k} className="flex items-center justify-between rounded-md border border-ink-200 bg-surface px-3 py-2">
        ...
        {topLevelCats.length > 0 && (
          <div className="grid grid-cols-2 gap-2 lg:grid-cols-4">
            {topLevelCats.map((c) => (
              <label
                key={c.cid}
                className="flex cursor-pointer items-center gap-2 rounded-md border border-ink-200 bg-surface px-3 py-2 hover:bg-ink-50"
              >
                <input
                  type="checkbox"
                  checked={!!c.tracked}
