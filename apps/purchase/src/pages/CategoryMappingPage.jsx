            ...
            <button
              ...
              className={`px-4 py-2 rounded-lg text-sm font-medium transition ${
                tab === t.id
                  ...
                  : 'bg-surface border border-ink-200 text-ink-600 hover:bg-ink-50'
              }`}
            >
              {t.label}
            </button>
                ...
                {(data?.items || []).map((row) => (
                  <tr key={row.id} className="border-t border-ink-100">
                    ...
                    <td className="px-3 py-2 text-right">
                      ...
                      <button
                        className="text-signal-err text-xs"
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="border rounded p-3">
          <div className="flex items-center justify-between mb-2">
            ...
            <span className={`text-xs px-2 py-0.5 rounded ${
              row.ai_naver_score >= 70 ? 'bg-soft-ok text-signal-ok'
                : row.ai_naver_score >= 50 ? 'bg-soft-warn text-signal-warn'
                  : 'bg-soft-err text-signal-err'
            }`}>
        ...
        <div className="border rounded p-3">
          <div className="flex items-center justify-between mb-2">
            ...
            <span className={`text-xs px-2 py-0.5 rounded ${
              row.ai_coupang_score >= 70 ? 'bg-soft-ok text-signal-ok'
                : row.ai_coupang_score >= 50 ? 'bg-soft-warn text-signal-warn'
                  : 'bg-soft-err text-signal-err'
            }`}>
      ...
      <div className="max-h-48 overflow-y-auto space-y-1">
        {results.map((c) => (
          <button
            ...
            className="w-full text-left px-2 py-1 rounded text-xs hover:bg-surface border border-transparent hover:border-ink-200"
            onClick={() => onPick(c)}
          >
