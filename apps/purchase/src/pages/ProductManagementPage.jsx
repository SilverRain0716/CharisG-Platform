  ...
  return (
    <div className="flex items-center gap-1">
      <input
        ...
        autoFocus
        className="w-24 rounded border border-ink-300 px-1.5 py-0.5 text-sm outline-none focus:border-accent/40"
      />
      <button onClick={() => { onSave(row.id, Number(val)); setEditing(false); }} className="text-signal-ok text-xs font-bold">✓</button>
...
const COLS = [
  ...
  { key: 'ai_processed_at', label: 'AI', width: '60px',
    render: (v) => v ? <span className="text-signal-ok font-medium">✓</span>
