...
export default function GroupDetailPage() {
  ...
  if (!data) return <div className="p-8 text-sm text-signal-err">그룹을 찾을 수 없음</div>;
    ...
    <div className="space-y-6">
      <header className="flex items-start justify-between gap-4">
        <div>
          ...
          <div className="mt-1 flex flex-wrap items-center gap-3 text-xs text-ink-500">
            ...
            <span>옵션: <strong className={overLimit ? 'text-signal-warn' : 'text-ink-900'}>{data.child_count}</strong></span>
            <a
              ...
              <button
                ...
                className={`px-3 py-1.5 rounded-md text-xs border transition ${
                  channel === c
                    ...
                    : 'bg-surface text-ink-600 border-ink-200 hover:border-pa-300'
                }`}
              >
                {c}
              </button>
        ...
        <DataTable
          columns={[
            ...
            { key: 'skipped_count', label: '잘림', width: '70px',
              render: (v) => v ? <span className="text-xs text-signal-warn">−{v}</span> : '—' },
      ...
      {(extendDryRun || extendError) && (
        <Card title="🎁 옵션 통합 등록" padded>
          {extendError && <div className="text-sm text-signal-err">{extendError}</div>}
          {extendDryRun && (
            <div className="space-y-3">
              {/* mode 설명 라벨 */}
              <div className={`rounded-md border p-3 text-xs ${
                extendDryRun.mode === 'register'
                  ? 'bg-soft-info border-signal-info/40 text-signal-info'
                  ...
                  return (
                    <div key={ch} className="rounded-md border bg-ink-50 border-ink-200 p-3">
                      ...
                      {items.map((it, idx) => {
                        ...
                        return (
                          <div key={idx} className={`mt-1 text-xs space-y-0.5 ${ok ? '' : 'text-signal-err'}`}>
                            {ok ? (
                              <>
                                ...
                                {it.subordinate_count !== undefined && (
                                  <div>archive 대상: <strong className="text-signal-warn">{it.subordinate_count}건</strong></div>
                                )}
              ...
              {/* 영향 경고 + 실행 */}
              <div className="rounded-md bg-soft-warn border border-signal-warn/40 p-3 text-xs text-signal-warn">
        ...
        <Card title="✅ 옵션 통합 등록 결과" padded>
          <div className="grid grid-cols-1 gap-3 md:grid-cols-2 text-xs">
            {['smartstore', 'coupang'].map((ch) => {
              ...
              return (
                <div key={ch} className={`rounded-md border p-3 ${ok ? 'bg-soft-ok border-signal-ok/40' : 'bg-soft-err border-signal-err/40'}`}>
                  ...
                  {ok && (
                    <div className="mt-2">
                      ...
                      {ci.needs_reapproval && <div className="text-signal-warn">⚠ 쿠팡 재승인 검수 1~3일 대기</div>}
                    </div>
                  )}
                  {!ok && <div className="mt-2 text-signal-err">{ci.reason || ci.stage}</div>}
                </div>
              );
            })}
          </div>
      ...
      {previewKey && (
        ...
        >
          {previewQuery.isLoading ? (
            ...
            <div className="text-sm text-signal-err">조회 실패: {previewQuery.error.message}</div>
          ) : (
            <div className="space-y-3">
