              ...
              <button
                ...
                className={`px-3 py-1.5 rounded-full text-xs border transition ${
                  filter === f.k
                    ...
                    : 'bg-surface text-ink-600 border-ink-200'
                }`}
              >
                {f.label}
              </button>
...
function StatCard({ label, value, accent = false }) {
  return (
    <div className={`rounded-lg border p-3 ${accent ? 'bg-soft-warn border-signal-warn/40' : 'bg-surface border-ink-200'}`}>
      <div className="text-[11px] text-ink-500">{label}</div>
      <div className={`mt-1 text-xl font-semibold ${accent ? 'text-signal-warn' : 'text-ink-900'}`}>
