...
export default function CoupangPage() {
  ...
  const excludedCount = allItems.filter((r) => r.status === 'excluded' && /^MANDATORY/.test(r.error_message || '')).length;
  const pendingCount = pendingItems.length;
  const listedCount = listedItems.length;
  const visibleItems = tab === 'pending' ? pendingItems : tab === 'listed' ? listedItems : [];
