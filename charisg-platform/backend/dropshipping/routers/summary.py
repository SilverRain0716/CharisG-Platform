"""DS summary — Hub 카드용 요약. market 파라미터로 마켓별 조회."""
...
@router.get("/summary")
def ds_summary(market: str = Query(default="US")):
    with get_db() as conn:
        active = conn.execute(
        ...
        listed = conn.execute(
            "SELECT COUNT(*) c FROM listings WHERE status='listed' AND marketplace=?",
            (market,),
        ).fetchone()["c"]

        health = conn.execute(
            "SELECT odr, late_shipment_rate, cancel_rate, valid_tracking_rate "
            "FROM account_health WHERE marketplace=? ORDER BY id DESC LIMIT 1",
            (market,),
        ).fetchone()
        if not health:
            health = conn.execute(
