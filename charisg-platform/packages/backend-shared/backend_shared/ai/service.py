...
- 구성: [브랜드] + [핵심 상품명] + [주요 스펙 1~2개] (예: "Anker 고속 무선 충전 패드 15W Qi 호환")
- 브랜드명은 영문 그대로 유지하세요.
...
**중요**: optimized_title은 반드시 50자 이내여야 합니다.
- 구성: [브랜드] + [핵심 상품명] + [주요 스펙 1~2개]
- 모델번호 나열, 호환 차종 나열, 세부 규격 리스트는 제거
- 괄호() 최소화, 불필요한 수식어(프리미엄, 고급 등) 제거
- 원본이 길면 브랜드+핵심기능+용도만 남겨 축약

JSON으로만 답변하세요 (다른 텍스트 없이):
{{
  "optimized_title": "SEO 최적화된 상품명 (50자 이내 필수)",
